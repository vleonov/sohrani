...
/*
  ...
  * Correct( $data, $noParagraph=false ) -- откорректировать текст в соответствии с настройками класса
      - $data        -- корректируемый текст
      - $noParagraph -- реликтовый параметр, который никак не используется нигде =) [deprecated]
...
class typografica
{
  ...
  var $phonemasks = array(
                        array(
                              "/([^0-9])([0-9]{4})\-([0-9]{2})\-([0-9]{2}) ([0-9]{2}):([0-9]{2}):([0-9]{2})([^0-9])/",
                              "/([^0-9])([0-9]{4})\-([0-9]{2})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])(\([0-9\+\-]+\)) ?([0-9]{3})\-([0-9]{2})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])(\([0-9\+\-]+\)) ?([0-9]{2})\-([0-9]{2})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])8\-([0-9]{3})\-([0-9]{3})\-([0-9]{2})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])8\-([0-9]{3})\-([0-9]{3})\-([0-9]{4})([^0-9])/",
                              "/([^0-9])(\([0-9\+\-]+\)) ?([0-9]{3})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])(\([0-9\+\-]+\)) ?([0-9]{2})\-([0-9]{3})([^0-9])/",
                              "/([^0-9])([0-9]{3})\-([0-9]{2})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])([0-9]{2})\-([0-9]{2})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])([0-9]{1})\-([0-9]{2})\-([0-9]{2})([^0-9])/",
                              "/([^0-9])([0-9]{2})\-([0-9]{3})([^0-9])/",
                              "/([^0-9])([0-9]+)\-([0-9]+)([^0-9])/",
                              ),
                        array(
                              "\\1<nobr>\\2&ndash;\\3&ndash;\\4&nbsp;\\5:\\6:\\7</nobr>\\8",
                              "\\1<nobr>\\2&ndash;\\3&ndash;\\4</nobr>\\5",
                              "\\1<nobr>\\2&nbsp;\\3&ndash;\\4&ndash;\\5</nobr>\\6",
                              "\\1<nobr>\\2&nbsp;\\3&ndash;\\4&ndash;\\5</nobr>\\6",
                              "\\1<nobr>8&ndash;\\2&ndash;\\3&ndash;\\4&ndash;\\5</nobr>\\6",
                              "\\1<nobr>8&ndash;\\2&ndash;\\3&ndash;\\4</nobr>\\5",
                              "\\1<nobr>\\2&nbsp;\\3&ndash;\\4</nobr>\\5",
                              "\\1<nobr>\\2&nbsp;\\3&ndash;\\4</nobr>\\5",
                              "\\1<nobr>\\2&ndash;\\3&ndash;\\4</nobr>\\5",
                              "\\1<nobr>\\2&ndash;\\3&ndash;\\4</nobr>\\5",
                              "\\1<nobr>\\2&ndash;\\3&ndash;\\4</nobr>\\5",
                              "\\1<nobr>\\2&ndash;\\3</nobr>\\4",
                              "\\1<nobr>\\2&ndash;\\3</nobr>\\4",
                              )
                         );
  
  var $glueleft = array( "рис\.", "табл\.", "см\.", "им\.", "ул\.", "пер\.", "кв\.", "офис", "оф\.", "пл\.", "ст\.", "т\.", "тел\.", "т\/ф\.", "т\/факс", "№");
  var $glueright = array( "руб\.", "коп\.", "у\.е\.", "USD", "EURO", "RUB", "мин\.", "час", "сек", "мес\.", "г\.", "гг\." );
  ...
  function correct( $data, $noParagraph=false )
  {
    ...
    if ($this->settings["wordglue"])
    {
      ...
      foreach ($this->glueleft as $i) {
         $data = preg_replace( "/([".$this->p."]+)(".$i.")(\s+)([a-zA-Zёа-яЁА-Я0-9]+)/i", "\\1\\2&nbsp;\\4", $data );
      }
      foreach ($this->glueright as $i)  {
        $data = preg_replace( "/([a-zA-Zёа-яЁА-Я0-9\-]+)(\s+)(".$i.")([".$this->p."]+)/i", "\\1&nbsp;\\3\\4", $data );
      }
    }
    // 4a. Ф.И.О.
    $data	= preg_replace("/([a-zA-Zёа-яЁА-Я]{2,})\s([A-ZЁА-Я]\.[A-ZЁА-Я]\.)/", "$1&nbsp;$2", $data);
    $data	= preg_replace("/([A-ZЁА-Я]\.[A-ZЁА-Я]\.)\s([a-zA-Zёа-яЁА-Я]{2,})/", "$1&nbsp;$2", $data);
...
//    if ($this->de_nobr) $data = str_replace( "<nobr>", "<span class=\"nobr\">", str_replace( "</nobr>", "</span>", $data ));
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	var	$s		= "\"";	//собственно символ, который будем заменять
	var	$p		= "-\",.!?;:\r\n\s\t\200="; //символы, которые не могут быть в словах
	var $rl		= "&laquo;";	//левая внешняя русская кавычка
	var $rli	= "&#132;";	//левая внутренняя русская кавычка
	var $rr		= "&raquo;";	//правая внешняя русская кавычка
	var $rri	= "&#147;";		//правая внутренняя русская кавычка
	var $el		= "&#147;";		//левая внешняя не русская кавычка
	var $eli	= "&#147;";		//левая внутренняя не русская кавычка
	var $er		= "&#148;";		//правая внешняя не русская кавычка
	var $eri	= "&#148;";		//правая внутренняя не русская кавычка
	var $c		= "\xa5";		//символ, на который временно заменяется исходная кавычка, будем надеяться он в тексте не встречается...
  
  function qReplace($str, $k, $f=false) {
	$j = 0;
	for ($i = $k; $i>0; $i--) {
	while (preg_match("/".$this->c.$i."(.*?)".$this->c.$i."/", $str, $rez) && $j++<100) {
		if (preg_match("/^(.*[ёа-яЁА-Я]+.*|[^a-zA-Z]*)$/", $rez[1])) {
			$l = ($f ? $this->rl : $this->rli); $r = ($f ? $this->rr : $this->rri);
		} else {
			$l = ($f ? $this->el : $this->eli); $r = ($f ? $this->er : $this->eri);
		}
		$str = preg_replace("/".$this->c.$i.".*?".$this->c.$i."/", "".$l.$this->qReplace($rez[1], $i-1, !$f).$r."", $str, 1);
	}
	}
	return $str;
  }
  ...
  function replaceSpecials( $data )
  {
    ...
    //1, 2. Лапки, елочки, ...
		...
		$data	= " ".$data." ";
		$k = 0;
		while (preg_match("/[".$this->p."]".$this->s."[^".$this->s."]*".$this->s."[".$this->p."]/", $data) && $k++<100) {
			$data = preg_replace("/([".$this->p."])".$this->s."([^".$this->s."]*)".$this->s."([".$this->p."])/", "$1".$this->c.$k."$2".$this->c.$k."$3", $data);
		}
		for ($i = $k; $i>0; $i--) {
			$data = $this->qReplace($data, $i, true);
		}
		$data	= preg_replace("/".$this->c."[0-9]+/", $this->s, $data);
...
/*    if ($this->settings["quotes"])
    ...
    if ($this->settings["dash"])
      $data = preg_replace( "/([0-9])\-([0-9])/i", "\\1&#150;\\2", $data );
    // 3a. тире длинное
    if ($this->settings["emdash"]) {
      $data = preg_replace( "/(\s|nbsp;)(\-\-|\–\–)(\s|&nbsp;)/i", "\\1&#151;\\3", $data );
      $data = preg_replace( "/(\s|nbsp;)(\-|\–)(\s|&nbsp;)/i", "\\1&#151;\\3", $data );      
    }
    // 3b. дефис
    if ($this->settings["dash"])
      $data = preg_replace( "/(\S)\-/i", "\\1-", $data );
